...
A production grade VPC built from scratch on AWS, demonstrating corenetworking concepts including public/private subnet isolation,
multi-AZ resillience, bastion host access patterns, and security group layering.
...
- 2 public subnets accross eu-west-2a and eu-west-2b
- 2 private subnets accross eu-west-2a and eu-west-2b
...
The AWS default VPC is pre configured and fine for testing, but real workloads need network isolation you control. Building custom means you understand every route, every route, and every boundary.

**Why two Availability Zones?**
A single Availability Zone (AZ) is a single point of failure. Spreading accross eu-west-2a and eu-west-02b means if one AWS data centre has an isuue, the architecture survives. This is the foundation of resillience on AWS.
...
**Why bastion host?**
The private instance has no public IP, there is no direct path from the internet to it. The bastion (public EC2) acts as a controlled jump point. You SSD into the bastion, then hop to the private instance from inside the VPC. This is standard practice for accessing private infrastructure.
...
The moment that landed the hardest: SSHing into the public server, then hopping into the private server using only its private IP. Seeing that the private instance was genuinely unreachable from the internet but accessible from inside the VPC made the whole subnet isolation concept real.
...
## Solutions Architect Asssociate Domains Covered
- Design Resillient Architectures (multi AZ, subnet isolation)
...
**Project 2 - The Scaleable Web App**
Taking this VPC and adding an Application Load Balancer, Auto ~Scaling Group, and RDS instance. Making it production grade.

*Part of [Ishola's SAA Build Path](https://github.com/SholaAkande) - buiulding towards AWS Solutions Architect Associate through projects, not passive study.*
